...
const STORAGE_BUCKET = 'mission-images';
...
function getPublicStorageUrl(fileName) {
  ...
  const hasExtension = /\.(jpg|jpeg|png|webp)$/i.test(raw);
  const finalName = hasExtension ? raw : `${raw}.jpg`;
...
function singularize(token) {
  ...
  for (const suffix of ['en', 'er', 'e', 'n', 's']) {
    if (token.length > suffix.length + 2 && token.endsWith(suffix)) variants.add(token.slice(0, -suffix.length));
  }
  return [...variants];
}
...
function answerMatches(input, answers) {
  ...
  for (const variant of singularize(inputNorm)) if (expandedAnswers.has(variant)) return true;
...
async function getFragmentsWithProgress() {
  const fragments = await sbFetch('/fragments?select=id,question,type,answer_main,answer_alternatives,reference_image_url,intro_text,intro_image_name,is_gift,is_active,puzzle_numbers&order=id.asc', { service: true });
  ...
  return fragments.map((f) => ({
    ...f,
    intro_image_url: getPublicStorageUrl(f.intro_image_name),
